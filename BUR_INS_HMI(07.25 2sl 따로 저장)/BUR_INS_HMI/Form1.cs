...
namespace BUR_INS_HMI
{
    public partial class Form1 : Form
    {
        ...
        private void SaveDataToCSV(ushort[] data1, ushort[] data2)
        {
            ...
            using (StreamWriter sw = new StreamWriter(csvFilePath, true, Encoding.UTF8))
            {
                ...
                // data1과 data2를 합쳐서 하나의 리스트로 만듦
                var allData = data1.Concat(data2);

                string line = timestamp + "," + string.Join(",", allData.Select(d => d.ToString()));
        ...
        private void SaveDataToSQL(ushort[] data1, ushort[] data2)   //미완
        {
            /*    try
                {
            ...
            try
            {
                if (data1.Length < 32 || data2.Length < 4)
                    return;

                int[] intData1 = data1.Take(32).Select(x => (int)x).ToArray();
                int[] intData2 = data2.Take(4).Select(x => (int)x).ToArray();

                int[] allData = intData1.Concat(intData2).ToArray(); // 총 36개

                mssql.InsertDB(allData);  // 저장
            }
            catch (Exception ex)
            {
                Console.WriteLine("DB 저장 실패: " + ex.Message);
        ...
        private void UpdateBarChart(ushort[] currentData)   //평균값 그래프
        {
            ...
            if (avg_plot == null || currentData == null || currentData.Length < 27)
                return;
        ...
        private async Task ReadModbusData(CancellationToken token)
        {
            ...
            for (int i = 0; i < deviceAddress.Length; i++)
            {
                ...
                ushort numberOfPoints = numInputs[i];
                        ...
                        case 1:
                            {
                                BeginInvoke(new Action(() =>
                                {
                                    ...
                                    if (data[6] == 1)
                                    {
                                        try
                                        {
                                            MessageBox.Show("LOCAL PANEL : estop 요청");
                                            _ = StopCommunicationAsync();
                                            estop = true;
                                        ...
                                        {
                                            MessageBox.Show("통신 정지 오류 : " + ex.Message);
                                        }
                                    }
            ...
            SaveDataToCSV(data1, data2);
            SaveDataToSQL(data1, data2);
        }
        ...
        private void sen_img(int[] stat)    //Red,Yellow,Green 기준은 추후 수정
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action(() => sen_img(sen)));
                return;
            }
        ...
        private async void pic_stop_btn_Click_1(object sender, EventArgs e)   //추후 데이터 받아서 estop시 반영 필요
        {
            if (serialPort != null && serialPort.IsOpen)    //포트 연결 되어 있고 통신도중일 때.
            {
                if (!estop)
                {
                    ...
                    {
                        pic_stop_btn.Image = BUR_INS_HMI.Properties.Resources.estop_on;
                        _modbusMaster.WriteSingleRegister(deviceAddress[0], 10, 0); //PC->LOCAL estop ON (LED OFF)
                ...
                else // estop == true 인 경우
                {
                    if (MessageBox.Show("비상 정지를 해제하시겠습니까?", "종료 확인", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        _modbusMaster.WriteSingleRegister(deviceAddress[0], 10, 1); //PC->LOCAL estop OFF (LED ON)
                        estop = false;
                    }
                }
