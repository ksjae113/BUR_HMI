...
namespace BUR_INS_HMI
{
    public partial class Form1 : Form
    {
      ...
      /*  private string settingfilePath = @"C:\Users\tjlee\Desktop\ksj\settings.txt";    //설정값 메모장 경로
        ...
        private string settingfilePath = @"C:\Users\User\Desktop\settings.txt";   //설정값 메모장 경로
        ...
        internal byte[] deviceAddress = { 4, 1, 2, 3 };  //Slave Number
        ...
        public CancellationTokenSource cancellationTokenSourceForModbus;
        ...
        private void InitializeGraphs()
        {
            ...
            // 레벨선 초기화
            double level = Convert.ToDouble(info_lbl9.Text);    //level = 지시 레벨링값
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                var lbl = new Label();    //Line Number 라벨
            ...
            /*   Color[] colors =
       {
        ...
        private void update_amp_temp(ushort[] data, int index)  //전류값 확인 및 모듈 상태, 온도 업데이트 함수
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    temp1[i + offset] = data[8 * i + 4] * 0.1M; //온도1 업데이트
                    temp2[i + offset] = data[8 * i + 5] * 0.1M; //온도2 업데이트
                }
            }
        }
            ...
            while (!token.IsCancellationRequested)
            {
...

                {
                    ...
                    try
                    {
                        ...
                        //Random rnd = new Random();  //데이터 랜덤 생성   //테스트용
                        ...
                        switch (i)
                        {
                            case 1:
                                ...
                                Invoke(new Action(() =>
                                {
                                    ...
                                    if (slave1Data[7] != target_rpm[1])//RPM 주기 실제 데이터와 동일한지 비교
                                    {
                                        _modbusMaster.WriteSingleRegister(deviceAddress[i],7, (ushort)target_rpm[1]);
                                    }    
                            ...
                            case 2:
                                {
                                    ...
                                    Invoke(new Action(() =>
                                    {
                                        ...
                                        getPosition();  //교정중 판정 함수
                                        if (slave2Data[7] != target_rpm[1]) //RPM 주기 실제 데이터와 동일한지 비교
                                        {
                                            _modbusMaster.WriteSingleRegister(deviceAddress[i], 7, (ushort)target_rpm[1]);
                                        }
        ...
        private void show_warning(string message)   //점멸하는 경고창
        {
            ...
            isWarning = true;   //true 변경으로 경고창 활성화 표시 ON  //연속 표시 방지용
            isBlink = true; //깜빡임 여부 (확인창 클릭 여부)
            string timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");    //경고 기준 시간 
            blinkTimer = new System.Windows.Forms.Timer();  //점멸용 타이머 생성
            blinkTimer.Interval = 1000; //점멸주기 1초
            blinkTimer.Tick += (s, e) =>    //틱당(1초) 변화
            {
                if (message == "RED") //넘겨받은 message 따라 경고창 색 결정
                {
                    
                    warning_pan.Location = new Point(700, 350);
                    
                    warning_pan.Visible = true;
                    //    warning_pan.Dock = DockStyle.Right;
                    warning_pan.Parent = this;  //다른 컨트롤들에 가려져서 Parent 재지정
                    warning_pan.BringToFront();
                    warning_time.Text = $"{timestamp}"; //경고 기준 시간 출력
                ...
                isFlashing = !isFlashing;   //isFlashing 반전으로 점멸 O X반복 [RED(YELLOW) or BLACK 화면 반복]
            };
            blinkTimer.Start();
        }
        private void warning_btn_Click(object sender, EventArgs e)  //경고창 확인 버튼
        {
            isBlink = false;    //[확인] 버튼 클릭시 점멸 중지하고 흑색 화면
        }

        private void warning_cls_btn_Click(object sender, EventArgs e)  //경고창 X 버튼
        {
            //[확인] 버튼 클릭 이후 X로 경고창 종료시 정상 종료
            //함수 재호출 가능하도록 isBlink, isWarning 다시 되돌리기
        ...
        {
            if ("TRUE".Equals(sender.ToString()))   //입력된 string값이 기존 패스워드와 동일한 경우 모듈 설정창 ON
            {
                f3.login_stat = 1;
          ...
          /*  if (serialPort != null && serialPort.IsOpen)    //포트 연결 되어 있고 통신도중일 때.
            {
                if (!estop)
                {
                    _modbusMaster.WriteSingleRegister(deviceAddress[3], 10, 1); //D3 ON
                ...
                else // estop == true 인 경우
                {
                    ...
                    {
                        _modbusMaster.WriteSingleRegister(deviceAddress[3], 10, 0); //D3 OFF
        ...
        //    저장 후 다시 읽어서 검증
            //string[] saved = File.ReadAllLines(filePath);

            //if (saved.Length == lines.Count &&
            //    saved.SequenceEqual(lines))
            //{
            //    MessageBox.Show("설정값이 정상적으로 저장되었습니다.", "Settings saved");
            //}
            //else
            //{
            //    MessageBox.Show("저장된 값이 손상되었거나 다릅니다.", "Error");
            //}
        ...
        private void warning_tmp_Click(object sender, EventArgs e)
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action(() => warning_tmp_Click(sender, e)));
                return;
            }
            ...
            if (isWarning == false) //[isWarning + @] 조건 설정으로 경고창 띄우는 함수 실행
            {
                ...
                show_warning("RED");    //점멸하는 경고창 "RED" or "YELLOW" 인수 전달로 경고창 2가지 case
            }
            //else if (isWarning == false &&)
            //{
            //    show_warning("YELLOW");
