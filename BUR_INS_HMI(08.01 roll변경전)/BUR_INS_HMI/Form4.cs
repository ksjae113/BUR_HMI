     ...
     /*   private void editer_btn_1_Click(object sender, EventArgs e)
        ...
        private void editer_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (unit_lbl.Text == "PERIOD")
                {
                    if (int.TryParse(input, out int periodValue))
                    {
                        // 10 ~ 60000 범위이며, 10의 배수인지 확인
                        if (periodValue >= 10 && periodValue <= 60000 && periodValue % 10 == 0)
                        {
                            editer_btn_check_Click(sender, EventArgs.Empty);
                        }
                        else
                        {
                            MessageBox.Show("주기는 10단위로 입력하세요 (최소 10ms ~ 최대 60000ms)");
                        }
                    }
                    else
                    {
                        MessageBox.Show("유효하지 않은 값이 포함되어 있습니다.");
                    }
                    editer.Text = null;
                }
                else
