     ...
     //   internal void update_Realamp(decimal[] a, int index)
        ...
        internal void update_temp(ushort[] data , int index)  //온도는 2개임.수정 완    //stop시 변경은 init으로 따로만들어야할듯
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                */
                ...
                // 차이가 difference값 이상 나는 경우만 데이터값 텍스트 UI 반영
                if (Math.Abs(data[i * 8 + 4] * 0.1M - Convert.ToDecimal(temp1_arr[i + offset].Text)) >= _mainForm.difference[0])
                {
                    temp1_arr[i + offset].Text = (data[i * 8 + 4] * 0.1).ToString("F1");
                }

                if (Math.Abs(data[i * 8 + 5] * 0.1M - Convert.ToDecimal(temp2_arr[i + offset].Text)) >= _mainForm.difference[0])
                {
                    temp2_arr[i + offset].Text = (data[i * 8 + 4] * 0.1).ToString("F1");
                }

             //   temp1_arr[i + offset].Text = (data[i * 8 + 4] * 0.1).ToString("F1");
             //   temp2_arr[i + offset].Text = (data[i * 8 + 5] * 0.1).ToString("F1");
        ...
        private void DiseaseUpdateEventMethodF4toF3(object sender, object unit)  //f4 입력에 맞춰 업데이트
        {
            ...
            else if ("PERIOD".Equals(unit.ToString()))
            {
                ...
                {
                    MessageBox.Show("장치가 연결되어 있지 않으므로 해당 수정값은 현재 저장 후 장치 연결시 적용됩니다.");
                    target_rpm[1] = Convert.ToDecimal(sender.ToString());
                    tar_period.Text = target_rpm[1].ToString();
                }
