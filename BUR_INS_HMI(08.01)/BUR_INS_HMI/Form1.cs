...
namespace BUR_INS_HMI
{
    public partial class Form1 : Form
    {
        ...
        internal decimal[] target_rpm = new decimal[] { 20, 2000 };  //[ 구동 판정 속도 | 구동 속도 주기 ]
        //임의로 Visible하게 : 20 -> 0                                                             
        ...
        private void UpdateBarChart(ushort[] currentData, int index)   //평균값 그래프
        {
            ...
            plt.XTicks(Enumerable.Range(0, groupLabels.Length).Select(i => (double)i).ToArray(), groupLabels);
            plt.SetAxisLimits(xMin: -0.5, xMax: layout.Length - 0.5, yMin: -1, yMax: Convert.ToDouble(info_lbl9.Text) * 2);
            ...
            /*   Color[] colors =
       {
            ...
            for (int i = 0; /*i < 1*/ i < deviceAddress.Length; i++)
            {
                ...
                try
                {
                    ...
                    ushort[] data = await Task.Run(() =>
                    {
                        ...
                        return _modbusMaster.ReadHoldingRegisters(deviceAddress[i], startAddress, numInputs[i]);
                        //Holding으로 바꿈
                    });
                    ...
                    _modbusMaster.WriteSingleRegister(deviceAddress[0], 7, 1000); //자꾸 10, 1000 갈려서 일단
        ...
        private List<ushort> rpmMeasurements = new List<ushort>();
        private List<ushort> ampMeasurements = new List<ushort>();
        private List<ushort> tempMeasurements = new List<ushort>();

        private double rpmReference = 0;   // 기준 RPM
        private double ampReference = 0;   // 기준 전류
        private double tempReference = 0;  // 기준 온도
        private void MeasureOn_btn_Click(object sender, EventArgs e)
        {
            rpmMeasurements.Clear();
            ampMeasurements.Clear();
        ...
        private void MeasurOff_btn_Click(object sender, EventArgs e)
        {
            MeasureStart = false;

            rpmReference = CalculateMedian(rpmMeasurements);
            ampReference = CalculateMedian(ampMeasurements);
            tempReference = CalculateMedian(tempMeasurements);

            MessageBox.Show($"기준 RPM: {rpmReference}, 기준 전류: {ampReference}, 기준 온도: {tempReference}");
        }
        private void OnDataReceived(ushort[] prevData)
        {
            if (!MeasureStart) return;

            // 예시 인덱스 매핑 (사용자 장비 구조에 맞게 수정 필요)
            // RPM: 0~39, TEMP: 40~59, AMP: 60~79
            for (int i = 0; i < 40; i++)
                rpmMeasurements.Add(prevData[i]);

            for (int i = 40; i < 60; i++)
                tempMeasurements.Add(prevData[i]);

            for (int i = 60; i < 80; i++)
                ampMeasurements.Add(prevData[i]);
        }
