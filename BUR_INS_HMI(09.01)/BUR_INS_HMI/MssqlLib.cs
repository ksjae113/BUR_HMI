...
namespace BUR_INS_HMI
{
    internal class MssqlLib
    {
           ...
           private static readonly string connectionString = "Server=DESKTOP-4J780GL\\MSSQLSERVER_22;" +
              "Database=sampledb;Uid=sa;Pwd=0000;";   //사무실 작업용 

     //   private static readonly string connectionString = "Server=DESKTOP-2K8H0IE;" +
     //         "Database=sampledb;Uid=sa;Pwd=0000;"; //RIST PC용
        ...
        /*    columns.Add("[판 식별번호]");
            ...
            columns.Add("[판 두께]");
            columns.Add("[판 길이]");
            columns.Add("[압연 종료 온도]");
            columns.Add("[가속 냉각 종료 온도]");
            columns.Add("[교정기 실적 Gap]");
            columns.Add("[지시 레벨링 속도]");*/
          ...
          /*  List<string> validColumns = new List<string>()    //이게 더 안전 추후 이거로  ?
    {
        ...
        // ... 나머지 허용 컬럼들 모두 추가
            ...
            string sql = $"SELECT TOP 1000 * FROM ModbusLog {whereClause} ORDER BY LogTime ASC";
