...
//        string folderPath = @"C:\Users\User\Desktop\storage";
            ...
            /*   Color[] colors =
       {
        ...
        private async Task ReadModbusData(CancellationToken token)  //데이터 읽어오기 
        {
            ...
            ushort[] plate = new ushort[9]; //판 정보용 크기 9 임시 배열
            /*    for (int i = 0; i < data.Length; i++)
                {
            ...
            while (!token.IsCancellationRequested)
            {
                for (int i = 0; i < 4; i++) //Slave 4, 1, 2, 3 

                {
                    ...
                    try
                    {
                        ...
                        plate[8] = (ushort)(rnd.Next(0, 301));  //임시 지시 Leveling 속도 0 ~ 300 난수값
                        ...
                        switch (i)
                        {
                            case 0:
                                ...
                                Invoke(new Action(() =>
                                {
                                    update_info(plate_data);    //판 정보용 함수
                                }));
                                break;
                ...
                // 🟡 슬레이브1과 슬레이브2 데이터 모두 수신되었을 경우   //i = 1 시 Plate null이라 데이터 미저장됨.
