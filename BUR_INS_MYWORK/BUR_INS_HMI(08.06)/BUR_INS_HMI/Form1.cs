    ...
    public partial class Form1 : Form
    {
        ...
        private void SaveDataToSQL(ushort[] data)   //미완
        {
            try
            {
                ...
                // ushort[] → string[] 변환
                string[] strData = data.Take(80).Select(x => x.ToString()).ToArray();
        ...
        private void UpdateBarChart(ushort[] currentData, int index)   //평균값 그래프
        {
             ...
             double[] groupAverages = new double[layout.Length]; // layout.Length = Line 개수 (예: 10)

             int dataIndex = 0;
             for (int i = 0; i < layout.Length; i++)
             {
                 double groupSum = 0;
                 int groupCount = layout[i]; // 예: 각 Line당 채널 수

                 for (int j = 0; j < groupCount; j++)
                 {
                     if (dataIndex >= currentData.Length) break;
                     groupSum += currentData[dataIndex++] / 10.0;  // 687 → 68.7
                 }
             ...
             plt.SetAxisLimits(xMin: -0.5, xMax: layout.Length - 0.5, yMin: -1, yMax: 1000);
...
/*
            ...
            plt.XTicks(Enumerable.Range(0, groupLabels.Length).Select(i => (double)i).ToArray(), groupLabels);
            plt.SetAxisLimits(xMin: -0.5, xMax: layout.Length - 0.5, yMin: -1, yMax: 500 //Convert.ToDouble(info_lbl9.Text) * 2);
            ...
            /*   Color[] colors =
       {
            ...
            try
            {
                if (serialPort == null || !serialPort.IsOpen)
                {
                    ...
                    try //장치 정상 연결 되어있는지 데이터 하나 읽어보기로 판단
                    {
                      ...
                      /*  ushort[] response = _modbusMaster.ReadHoldingRegisters(3, 0, 1);    //BASSO 용
        ...
        private async Task ReadModbusData(CancellationToken token)
        {
           ...
           // for (int i = 2; i < 3 /*i < deviceAddress.Length*/; i++)
           //     for (int i = 1; i < 3 /*i < deviceAddress.Length*/; i++)
                 for (int i = 2; i > 1; i--)
                {
                ...
                try
                {
                    ushort[] data = await Task.Run(() =>
                    {
                     ...
                     //      return _modbusMaster.ReadInputRegisters(3, 0, 14); //BASSO용
                        return _modbusMaster.ReadHoldingRegisters(deviceAddress[i], startAddress[i], numInputs[i]);
                        //Holding으로 바꿈
                    });
                 ...
                 //   _modbusMaster.WriteSingleRegister(deviceAddress[1], 7, (ushort)target_rpm[1]);
      ...
      /*  public void modify_data(ushort[] data)    //ushort므로 10나누기 X. 데이터 저장할 때 따로 해줘야 할
        {
            for (int i =0; i< data.Length ; i ++)
            {
                if (i % 7 != 0)
        ...
        public void saveFirstData(ushort[] data)
        {
            for (int i = 0; i < 80; i++)
            {
                if (data[i] != 0)
        ...
        private void run_btn_Click(object sender, EventArgs e)
        {
            ...
            if (!portOpend)
            {
                MessageBox.Show("포트를 열 수 없습니다.", "PORT FAIL");
                return;
            }
        ...
        private void screencap_btn_Click(object sender, EventArgs e)
        {
            ...
            //   string desktopPath = @"C:\Users\tjlee\Desktop\ksj\Captured";
                ...
                string desktopPath = @"C:\Users\User\Desktop\ScreenCaptured";
         ...
         //   MessageBox.Show("None");
            if (serialPort != null && serialPort.IsOpen)    //포트 연결 되어 있고 통신도중일 때.
            {
                if (!estop)
                {
                    ...
                    {
                        pic_stop_btn.Image = BUR_INS_HMI.Properties.Resources.estop_on;
                        _modbusMaster.WriteSingleRegister(deviceAddress[2], 10, 1); //D3 ON  DataProcess 종료
        ...
        private bool MeasureStart = false;

        private List<ushort>[] channelMeasurements = new List<ushort>[80];

        private void MeasureOn_btn_Click(object sender, EventArgs e)
        {
            channelMeasurements = new List<ushort>[80];   // 배열 자체도 새로 할당
            for (int i = 0; i < 80; i++)
                channelMeasurements[i] = new List<ushort>();  // 각 리스트 초기화

            MeasureStart = true;
            MessageBox.Show("계측을 시작합니다.");
            MeasureOn_btn.Visible = false;
            MeasureOff_btn.Visible = true;
        }

        private void MeasureOff_btn_Click(object sender, EventArgs e)
        {
            MeasureStart = false;
            MessageBox.Show("계측을 종료합니다.");
            MeasureOn_btn.Visible = true;
            MeasureOff_btn.Visible = false;
            double[] medianValues = CalculateMedianPerChannel();

            for (int i = 0; i < 80; i++)
                MeasureData[i] = (ushort)Math.Round(medianValues[i]);

            // 예시: 채널별 중앙값 출력
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < medianValues.Length; i++)
            {
                sb.AppendLine($"데이터 {i}: {medianValues[i]:F1}");
            }
            MessageBox.Show(sb.ToString());
        }

        private double[] CalculateMedianPerChannel()
        {
            double[] medians = new double[80];
            for (int i = 0; i < 80; i++)
            {
                medians[i] = CalculateMedian(channelMeasurements[i]);
            }
            return medians;
        }

        private double CalculateMedian(List<ushort> data)
        {
           
            var filtered = data.Where(v => v != 0).OrderBy(v => v).ToList();    //0값 제외
            ...
            /*if (data.Count == 0) return 0;    //0값 미제외
        ...
        private void OnDataReceived(ushort[] prevData)
        {
            if (!MeasureStart) return;

            if (prevData == null || prevData.Length < 80) return;

            for (int i = 0; i < 80; i++)
            {
                channelMeasurements[i].Add(prevData[i]);
            }
        }

        private bool IsOutOfRange(double currentValue, double referenceValue, double percent)
        {
            double lower = referenceValue * (1 - percent / 100.0);
            double upper = referenceValue * (1 + percent / 100.0);
            return currentValue < lower || currentValue > upper;
            MessageBox.Show("오차 벗어남");
        }
