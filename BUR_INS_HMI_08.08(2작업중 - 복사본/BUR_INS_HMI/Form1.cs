...
namespace BUR_INS_HMI
{
    public partial class Form1 : Form
    {
        ...
        ushort[] startAddress = { 0x0000, 0x0000, 0x0010 };   // 30001
     ...
     //   string csvFilePath = @"C:\Users\tjlee\Desktop\ksj\storage\modbus_log.csv";
        ...
        private void InitializeGraphs()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                fp_array[i] = new FormsPlot();

                fp_array[i].Plot.SetAxisLimits(xMin: 0, xMax: xRangeSeconds, yMin: -1, yMax: 30);
                fp_array[i].Plot.Title($"{(i + 1).ToString("D2")}", size: 20);
             //   fp_array[i].Plot.Layout(left: 20, right: 5, top: 5, bottom: 20);
                fp_array[i].Plot.XAxis.TickLabelStyle(fontSize: 10);
                fp_array[i].Plot.YAxis.TickLabelStyle(fontSize: 10);
                ...
                plots.Add(fp_array[i]);
                this.flowLayoutPanel1.Controls.Add(fp_array[i]);
        ...
        bool isline = false;
        private void UpdateBarChart(ushort[] data, int index)   //평균값 그래프
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action(() => UpdateBarChart(data, index)));
                return;
            }
            ...
            /*   Color[] colors =
       {
        ...
        private async Task ReadModbusData(CancellationToken token)
        {
            ...
            // for (int i = 2;i < deviceAddress.Length; i++)
            {
              ...
              */
                ...
                {
                  //  Console.WriteLine($"ReadModbusData Slave {i} Exception: " + ex.Message);
                }
            }
