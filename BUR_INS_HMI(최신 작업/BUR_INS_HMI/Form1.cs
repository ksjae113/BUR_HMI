        ...
        private void Init_port()
        {
            ...
            try
            {
                if (serialPort == null || !serialPort.IsOpen)
                {
                    ...
                    run_btn.Image = BUR_INS_HMI.Properties.Resources.runon1;
                }
            }
            catch (Exception ex)
            {
      ...
      /*  private void copyroll(InfoPanel p)
        {
            ...
            if (InvokeRequired)
            {
                //  this.Invoke(new Action(() => copyroll(p)));
                BeginInvoke(new Action(() => copyroll(p)));
                return;
            }
        ...
        private void sen_img(int[] stat)    //Red,Yellow,Green 기준은 추후 수정
        {
            for (int i = 0; i < 10; i++)
            {
                if (stat[i] == 0)
                {
                    sensor[i].Image = BUR_INS_HMI.Properties.Resources.sen_stat_green_control1;
        ...
        private void ampare_btn_Click(object sender, EventArgs e)
        {
         ...
         //   f3.ampare_panel.Visible = true;
            f3.setting_panel.Visible = false;
            f3.err_set_btn.Visible = false;
            f3.tar_rpm_btn.Visible = false;
        ...
        private async Task StopCommunicationAsync()
        {
            try
            {
                ...
                if (serialPort != null && serialPort.IsOpen)
                {
                    ...
                    serialPort.Close();
                    MessageBox.Show("통신 종료됨");
                    run_btn.Image = BUR_INS_HMI.Properties.Resources.runoff1;
                }
        ...
        */
        ...
        private void textBox1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    MessageBox.Show("1 이상 정수를 입력하세요 (분 단위)");
                }
            }
            /* if (e.KeyCode == Keys.Enter)
             {
