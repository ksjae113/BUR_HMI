...
namespace BUR_INS_HMI
{
    public partial class Form3 : Form
    {
     ...
     //   internal InfoPanel infoCopy;
        ...
        public Form3(decimal[] f1amp, decimal[] temp, decimal[] std_amp)
        {
            InitializeComponent();
        ...
        private void init_amp()
        {
            ...
            tar_rpm.Text = target_rpm.ToString("F1");
            update_err_all();
        }
        ...
        internal void update_Realamp(decimal[] a)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                real_amp[i].Text = (a[i] * channels[i]).ToString("F1");
        ...
        internal void update_temp(decimal[] t)  //온도는 2개임. 추후 수정    //stop시 변경은 init으로 따로만들어야할듯
        {
            if (this.InvokeRequired)
            {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                if (Convert.ToDecimal(temp1_arr[i].Text) < 1)
                    temp1_arr[i].ForeColor = Color.Red;
                else
                    temp1_arr[i].ForeColor = Color.Blue;
                if (Convert.ToDecimal(temp2_arr[i].Text) < 1)
        ...
        private void DiseaseUpdateEventMethodF4toF3(object sender, object unit)  //f4 입력에 맞춰 업데이트
        {
            ...
            {
                target_rpm = Convert.ToDecimal(sender.ToString());
                tar_rpm.Text = target_rpm.ToString("F1");
                update_err_all();
